 ...
 * LC11 - 盛最多水的容器
 *
 * 题目（概要）：给定 n 个非负整数表示柱子高度，选两根柱子与 x 轴围成容器，求最大盛水面积。
 *
 * 算法原理（摘要）：
 * - 不变式：每次迭代时，存在至少一组最优解 (i*,j*) 满足 left<=i*<j*<=right。移动 shorter 端不会丢失该最优解。
 * - 必要性：面积=min(h[L],h[R])*(R-L)，移高的一边宽度减、高度仍被短边限制，面积不可能增大。
 * - 充分性：移短边后，若存在更优解则其端点必在新区间内（反证：以 L 为左端的容器面积上界为 A，不能更优），故不变式保持。
 *
 * 解法说明：双指针 left=0、right=n-1，每次算面积并移动较短边（面积由短边限制，移短边才有机会找到更高边界）。
 *
 * 时间复杂度：O(n)
 * 空间复杂度：O(1)
 *
